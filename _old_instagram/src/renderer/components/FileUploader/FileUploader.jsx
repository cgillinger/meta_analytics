...
export function FileUploader({ onDataProcessed, onCancel, existingData = null, isNewAnalysis = false }) {
  const [files, setFiles] = useState([]); // Array of { file, status, error, analysis, result }
  const [isDragging, setIsDragging] = useState(false);
  const [isProcessing, setIsProcessing] = useState(false);
  const [batchResult, setBatchResult] = useState(null); // { succeeded, failed, total }
  ...
  const { columnMappings, validateColumns } = useColumnMapper();
  ...
  const addFiles = useCallback(async (newFiles) => {
    ...
    // Analyze files for memory projection
    const analyzedEntries = await Promise.all(
      fileEntries.map(async entry => {
        try {
          ...
          return { ...entry, analysis, _content: content };
        } catch {
          return entry;
        }
      })
    ...
    // Update memory check with first file's analysis
  ...
  }, [existingFilesMetadata, isNewAnalysis, memoryUsage]);
  ...
  const processSingleFile = async (entry, shouldMerge, isFirst) => {
    try {
      // Read content if not already cached
      ...
      if (!validation.isValid && validation.missing && validation.missing.length > 0) {
        // Warn but continue (soft validation)
        console.warn('Saknade kolumner:', validation.missing.map(m => m.original).join(', '));
      }

      // If new analysis and first file, clear existing data first
  ...
  const handleProcessFiles = async () => {
    ...
    for (let i = 0; i < pendingFiles.length; i++) {
      ...
      if (result.success) {
        ...
        shouldMerge = true; // subsequent files always merge
        setFiles(prev => prev.map(f =>
            ...
            <div className="flex flex-col items-center space-y-3">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">
                  Du kan välja flera CSV-filer samtidigt. Data behandlas lokalt i din webbläsare.
                </p>
                {totalMemoryKB > 0 && (
                ...
                <div className="flex space-x-2">
                  {failedCount > 0 && (
                    <Button variant="outline" size="sm" onClick={retryFailed}>
                      ...
                      Försök igen med misslyckade ({failedCount})
                    </Button>
                  )}
