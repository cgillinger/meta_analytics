...
import { getUploadedFilesMetadata, removeFileMetadata, clearAllData } from '@/utils/storageService';
...
export function LoadedFilesInfo({ onRefresh, onClearAll, canClearData = true }) {
  ...
  const PlatformBadge = ({ platform }) => {
    ...
    const config = PLATFORM_LABELS[platform.toLowerCase()] || {
      label: platform,
      className: 'bg-gray-100 text-gray-700',
    };
    return (
      <span className={`inline-block text-xs font-medium px-1.5 py-0.5 rounded ml-2 ${config.className}`}>
        {config.label}
      </span>
    );
  };
